...
<title>报表平台_报表查询-座位汇总</title>
<jsp:include page="/projectreport/common/include.jsp"></jsp:include>
</head>
<body>
	<!--顶部 begin-->
	...
	<div class="mai_main clear">
		<!-- 左侧导航-->
		<c:import url="/projectreport/common/navLeft.jsp"></c:import>
		<!--右侧内容-->
		<div class="mai_wrap">
			<div class="right_ticket por">
				<div class="ticket_nav-bt">
					<ul class="ticket_nav fl">
						...
						<li><a
							href="<%=basePath%>seatStatDetailReport.do?projectId=<c:out value="${projectId}"/>&source=<c:out value="${source}"/>">明细</a></li>
					</ul>
					<div class="fr">
				...
				<c:if test="${seatList != null}">
						<table class="r-s-table mt10">
							...
							<tbody>
								...
								<c:forEach var="SeatStatVo" items="${seatList}">
									<tr>
									...
									<tr>
										...
										<td>￥<fmt:formatNumber value="${SeatStatVo.staffAmount}"
												pattern="#.##" /></td>
										<td>￥<fmt:formatNumber value="${SeatStatVo.protectAmount}"
												pattern="#.##" /></td>
										<td>￥<fmt:formatNumber
	...
	<!--内容区 end-->
	<form id="frmSearch" method="post"
		action="<%=basePath%>seatStatInfoReport.do">
		<input type="hidden" id="source" name="source" value="<c:out value="${source}"/>" /> <input
			type="hidden" id="action" name="action" value="0" />
	</form>
	<!--底部 begin-->
