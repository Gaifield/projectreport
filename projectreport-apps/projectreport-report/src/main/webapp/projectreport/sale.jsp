...
<div class="mai_main clear">
    ...
    <div class="mai_wrap">
        <div class="right_ticket por">
            <div class="ticket_nav-bt">
                <ul class="ticket_nav fl">
                    ...
                    <li><a href="javascript:void(0)"
                           onclick="skipToSaleDetail();">明细</a></li>
                </ul>
                <div class="fr">
            ...
            <div class="r-cnt">
                <div class="r-biaoti">
                    <span><c:out value="${saleVo.projectName}" ></c:out></span>
                    <a class="r-choose fr" href="javascript:;" onclick="showPop()">重选场次</a>
                </div>

                <form id="saleFormByTime" method="post" action="saleReport.do">
                    ...
                    <input type="hidden" name="performIds" value="<c:out value="${performIds}" ></c:out>">
                    <input type="hidden" name="page" value="<c:out value="${page}" ></c:out>">
                ...
                <div class="r-table-cnt mt10">
                    <table class="up-report-table">
                        ...
                        <tr>
                            ...
                            <th rowspan="2" width="140">可售总票房</th>
                        ...
                        <c:forEach var="saleRowVo" items="${saleVo.saleRowVoList}">
                            <tr>
                                <td width="110" rowspan="2"><c:out value="${saleRowVo.priceVo.priceShowName}" ></c:out></td>
                                <td width="110">数量（张）</td>
                        ...
                        <tr class="fb">
                            ...
                            <td width="139">${saleVo.staffQuantity}</td>
                            <td width="139">防涨票（张）：</td>
                            <td width="139">${saleVo.protectQuantity}</td>
                            <td class="tc" width="695" colspan="${saleVo.allDisaccountVoList.size()+4}">注：附加出票为场馆，公安工作票，防涨票出票</td>
                        </tr>
                        <tr class="fb">
                            <td class="tc">总计出票数量（张）：</td>
                            <td colspan="2" width="249"><c:out value="${saleVo.totalQuantity}" ></c:out></td>
                            <td colspan="2">
                                <div class="fix-fr">总计金额（元）：</div>
                            <td width="695" colspan="${saleVo.allDisaccountVoList.size()+4}">
                ...
                <!-- 场次弹层提交数据 -->
                <form id="frmSearch" method="post" action="<%=basePath%>saleReport.do">
...
<div class="choose-cc">
    ...
    <form id="frmSearchPerform">
        ...
        <div class="r-search pl10 clear">
            <label>按演出时间搜索：</label>
            <input class="inp-txt1 w157 Wdate"
                   type="text" id="startTime" name="startTime"
                   pattern="yyyy-`-dd HH:mm"
                   onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',maxDate:'#F{$dp.$D(\'endTime\');}'})">
            <span class="r-flag">至</span>
            <input class="inp-txt1 w157 Wdate"
...
<script type="text/javascript">
    //初始化表格样式
    $(function () {
        var disaccountSize = ${saleVo.allDisaccountVoList.size()+3};
        var disaccountWidht = disaccountSize * 100;
        var otherWidth = 140 * 4 + 120 * 2;
        $(".up-report-table").width(disaccountWidht + otherWidth);
    });
    //初始化查询条件
    $(function () {
    ...
    //跳转到出票汇总明细界面
    function skipToSaleDetail() {
        window.location = "<%=basePath%>saleReportDetail.do?projectId=${projectId}&source=${source}";
    }
